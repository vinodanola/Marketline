...
App.controller('reportNasabahCtrl', function (apiData, $scope, apiBase, $stateParams, $sessionStorage,globalFunction,$rootScope,$window,jasperBaseV2) {
	...
	$scope.reportPDF = function (prospekid) { 	
		...
		if ($rootScope.namaREPORT == 'SP3U'){					
			...
			$scope.URL = jasperBaseV2+document.getElementById('reportnya' + prospekid).value+'.jrxml&DB_PROSPEK_ID='+prospekid;			        		
...
App.controller('modalReportSp3uCtrl', function ($sce, $rootScope, $scope, $stateParams, apiData, apiBase, $sessionStorage, jasperBase, globalFunction, $window, jasperBaseV2) {   
	...
	$rootScope.viewModalRPT_SP3U = function () {			
		$scope.URL = jasperBaseV2+document.getElementById('reportnya' + $rootScope.PROSPEK_ID).value+'.jrxml&DB_PROSPEK_ID='+$rootScope.PROSPEK_ID;			        		
		$window.open($scope.URL);
	}
